...
int zgt_tm::TxRead(long tid, long obno, int thrNum)
 {
   ...
   struct param *nodeinfo = (struct param*)malloc(sizeof(struct param));
   nodeinfo->tid = tid; //assigning the transaction id
   nodeinfo->obno = obno; // assigning the object number
   nodeinfo->Txtype = ' '; // assigning the transaction type
   nodeinfo->count = --SEQNUM[tid];
   int status;
   status = pthread_create(&threadid[thrNum],NULL,readtx,(void*)nodeinfo); // fork a thread  and call readtx function
   if (status){
     printf("ERROR: return code from pthread_create() is:%d\n", status);
     exit(-1);
   }
...
int zgt_tm::TxWrite(long tid, long obno, int thrNum)
 {
    ...
    struct param *nodeinfo = (struct param*) malloc(sizeof (struct param));
    nodeinfo->tid = tid; // assigning the transaction id
    nodeinfo->obno = obno; // assigning the object no
    nodeinfo->Txtype = ' '; // assigning default transaction type
    nodeinfo->count = --SEQNUM[tid];
    int status;
    status = pthread_create(&threadid[thrNum], NULL, writetx, (void*) nodeinfo); // fork a thread and call write tx function
    if (status)
      {
        printf("ERROR: return code from pthread_create() is:%d\n", status); // if unable to create a thread print error
        fflush(stdout);
	exit(-1);
    }
...
int zgt_tm::CommitTx(long tid, int thrNum)
 {
   ...
   struct param *nodeinfo = (struct param*)malloc(sizeof(struct param)); // creating a node
   nodeinfo->tid = tid;//assigning transaction id
   nodeinfo->obno = -1; // assigning default object no
   nodeinfo->Txtype = ' ';   // assigning the default transaction type
   nodeinfo->count = --SEQNUM[tid];
   int status;
   status = pthread_create(&threadid[thrNum],NULL,committx,(void*)nodeinfo); // fork a thread and call committx function
   if (status){
     printf("ERROR: return code from pthread_create() is:%d\n", status); // if unable to create a thread print error
     fflush(stdout);
	exit(-1);
...
int zgt_tm::AbortTx(long tid, int thrNum)
 {
   ...
   nodeinfo->tid = tid; // assigning transaction id
   nodeinfo->obno = -1; // assigning default object no
   nodeinfo->Txtype = ' ';// assigning default transation type
   nodeinfo->count = --SEQNUM[tid];
   int status;
   status = pthread_create(&threadid[thrNum],NULL,aborttx,(void*)nodeinfo); // fork a thread and call aborttx function
   if (status){
     printf("ERROR: return code from pthread_create() is:%d\n", status); // if unable to create a thread print error
fflush(stdout);    
 exit(-1);
